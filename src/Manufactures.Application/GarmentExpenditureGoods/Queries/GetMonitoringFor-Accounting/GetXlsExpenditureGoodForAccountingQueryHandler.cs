...
{
	public class GetXlsExpenditureGoodForAccountingQueryHandler : IQueryHandler<GetXlsExpenditureGoodForAccountingQuery, MemoryStream>
	{
		...
		public GetXlsExpenditureGoodForAccountingQueryHandler(IStorage storage, IServiceProvider serviceProvider)
		{
			_storage = storage;
		...
		public async Task<MemoryStream> Handle(GetXlsExpenditureGoodForAccountingQuery request, CancellationToken cancellationToken)
		{
			...
			var querySum = Query.ToList().GroupBy(x => new { x.expenditureDate, x.expenditureGoodNo, x.invoice, x.comodityCode, x.comodityName, x.uomUnit, x.roNo, x.buyerArticle, x.expenditureGoodType, x.colour, x.buyerCode, x.name, x.unitname, x.fc }, (key, group) => new
			{
				ros = key.roNo,
			...
			foreach (var item in querySum)
			{
				...
				GarmentMonitoringExpenditureGoodDto dto = new GarmentMonitoringExpenditureGoodDto
				{
					...
					nominal = Convert.ToDecimal(item.qty) * item.price,
			...
			if (listViewModel.garmentMonitorings.Count > 0)
			{
				foreach (var report in listViewModel.garmentMonitorings)
				{
					...
					reportDataTable.Rows.Add(index,report.expenditureGoodNo, report.expenditureGoodType,ExpenDate,report.pebDate,report.roNo,report.buyerArticle,report.colour,
						report.name, report.unitname, report.price,report.qty,report.nominal,report.invoice
					);
					counter++;
					index++;
			...
			using (var package = new ExcelPackage())
			{
				...
				worksheet.Cells["A1"].Value = "LAPORAN MONITORING BON PENGIRIMAN BARANG JADI "; worksheet.Cells["A" + 1 + ":L" + 1 + ""].Merge = true;
				worksheet.Cells["A2"].Value = "Periode " + dateFrom.ToString("dd-MM-yyyy") + " s/d " + dateTo.ToString("dd-MM-yyyy");
                worksheet.Cells["A3"].Value = "Konfeksi " + request.unitname;
				...
				worksheet.Cells["A" + 5 + ":N" + 5 + ""].Style.Font.Bold = true;
				...
				}else
				{
					...
					worksheet.Cells["A" + (counter) + ":K" + (counter) + ""].Merge = true;
				}
